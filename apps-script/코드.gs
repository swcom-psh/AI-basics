...
- 한 단계에서 필요한 것은 한 메시지에 모아 묻고, 번호·기호로 짧게 답할 수 있게 한다.
...
2) 주제고르기 : 그 분야에서 주제 후보 3개를 한 줄씩 제안하고 고르게 한다. 후보 3개는 각각 아래 ①~⑤를 모두 통과해야 한다.
   ...
   - 시점만 바꾸면 통과하는 경우(예: 경기 뒤 기록 → 경기 전에 미리 예측)에는 퇴짜 놓지 말고 "언제 예측할까요? ①… ②…"로 예측 시점을 학생이 고르게 한다.
...
3) 구성정하기 : 주제가 정해지면 한 메시지에 모아서 보여 준다.
   - 유형(예측/분류), 맞히려는 값의 이름·단위, 대상(어느 가게·지역·게임·병동 하나인지), 예측 시점(언제 예측하는지). 분류면 두 범주와 기준(예: 3개월 안 재구매).
     예측 시점이나 분류 기준이 여러 가지로 가능하면 선택지 2~3개를 주고 학생이 고른다.
   - 쓸 정보 후보 5~6개에 번호를 붙여서. 학생이 3~4개를 고르거나 직접 추가한다. 후보는 숫자로 잴 수 있거나 예/아니오(0/1)인 것만.
     예측 시점에 아직 모르는 값(결과 뒤에 생기는 값)은 후보에 넣지 않는다. 내일 날씨처럼 미래 값은 "(예보)"를 붙일 때만 쓴다.
   - 예측이면 「도움 안 될 것 같은 값」 후보 2개. 범위가 정의로 정해지는 값으로 고른다(예: 날짜의 일 1~31, 전화번호 끝자리 0~9, 예/아니오).
     "모델이 쓸모없는 정보를 걸러 내는지 보려는 값"이라고 한 문장으로 알려 주고 학생이 하나 고른다.

4) 방향정하기 : 고른 쓸 정보마다 영향을 한 번에 묻는다.
   예측 — "각각 골라 주세요. ①기온 ②유동인구 ③강수량 / 보기: 크게↑ 조금↑ 조금↓ 크게↓"
     크게↑=강한양, 조금↑=약한양, 조금↓=약한음, 크게↓=강한음. 학생이 ↑/↓만 말하면 중간양/중간음으로 기록한다(세기를 다시 묻지 않는다).
     예/아니오 정보는 "해당할 때" 기준으로 묻는다. (예: "주말이면?")
   분류 — 숫자 정보는 "각각 어느 쪽이 더 클까요?", 예/아니오 정보는 "해당하는 경우가 어느 쪽에 더 많을까요?" 보기: 두 범주.
...
7) 요약확인 : 전체를 짧게 요약하고 "맞으면 '네'라고 해 주세요"라고 묻는다. 이유는 따옴표 안에 학생이 쓴 말 그대로 옮긴다.
...
- 절대 대신 정하지 않는다. 묶음(A안: ①②③ / B안: ①③⑤)을 만들어 주지도 않는다.
- 하나를 고르는 질문이면 선택지를 둘로 줄여 다시 묻는다. 학생이 조금이라도 반응한 것을 남기고, 없으면 앞의 두 개를 남긴다.
- 여러 개를 고르는 질문이면 "제일 중요해 보이는 것 하나만 먼저 골라 주세요"로 받고, 그다음은 "나머지는 한 번에 골라도 돼요"라고 묻는다.
- 숫자를 묻는 질문이면 숫자·보기를 주지 말고 대상 범위를 되묻는다. 이유를 묻는 질문이면 빈칸 문장을 준다.
- 대화 전체에서 이렇게 답한 것이 두 번 쌓이면(연속이 아니어도), 그 뒤로 하나를 고르는 질문은 처음부터 선택지 둘로 묻는다.
...
- 답변 : 학생에게 보일 말
- 단계 : 관심분야/주제고르기/구성정하기/방향정하기/범위찾기/이유작성/요약확인
...
- 검색요청 : 5단계에 처음 들어갈 때만, 검색이 필요한 항목의 [{항목, 검색어}]. 그 밖에는 빈 배열.
  검색어는 한국어로, 범위·평균·통계가 드러나게. 예) "입원 환자 복용 약 개수 평균 통계"
- 도움요청 : 세 번째 거부로 멈췄을 때만 true
- 확정가능 : 정리의 모든 칸(근거 포함)이 채워지고 주제점검이 통과이며 학생이 요약에 "네"라고 했으면 true
`.trim();
...
function 응답스키마_() {
  const 특성 = {
    ...
    properties: {
      ...
      높은쪽: { type: 'string', description: '분류일 때만. 이 값이 큰 쪽(또는 더 많은 쪽) 범주 이름. 아니면 빈 문자열' }
    }
  };
  return {
    type: 'object', additionalProperties: false,
    ...
    properties: {
      답변: { type: 'string' },
      단계: { type: 'string',
        enum: ['관심분야','주제고르기','구성정하기','방향정하기','범위찾기','이유작성','요약확인'] },
      확정가능: { type: 'boolean' },
      ...
      정리: {
        type: 'object', additionalProperties: false,
        required: ['트랙','주제','대상','예측시점','타깃','범주A','범주B','특성','무관변수','이유','기대효과'],
        properties: {
...
function 검색질문_(검색어) {
  return '"' + 검색어 + '" 를 웹에서 찾아서, 한국 고등학생이 데이터의 값 범위를 정할 때 참고할 수 있게 정리해 줘.\n' +
    '- 실제로 찾은 자료에 나온 숫자만 쓴다. 없으면 "자료를 찾지 못했습니다"라고만 쓴다.\n' +
    '- 3줄 이내. 줄마다 끝에 (자료 이름) 을 붙인다.\n' +
    '- 마지막 줄은 "대략 범위: 최소 ~ 최대 단위" 한 줄. 자료로 알 수 없는 쪽은 ? 로 둔다.\n' +
    '- 숫자나 출처를 지어내지 않는다. 한국 자료를 먼저 찾는다.';
}
  ...
  if (req.일 === '목록') {
    ...
    for (let r = 1; r < v.length; r++) {
      ...
      행.push({
        ...
        트랙: v[r][7], 주제: v[r][8], 대상: v[r][9], 예측시점: v[r][10],
        타깃: { 이름: v[r][11], 단위: v[r][12], 최소: v[r][13], 최대: v[r][14], 근거: v[r][15] },
        범주A: v[r][16], 범주B: v[r][17],
        특성: f,
        무관: { 이름: v[r][46], 단위: v[r][47], 최소: v[r][48], 최대: v[r][49] },
        이유: v[r][50], 기대효과: v[r][51],
        점검: v[r][52], 점검메모: v[r][53],
        대신요청: v[r][54], 도움요청: v[r][55],
        주고받은수: v[r][56]
      });
    }
...
function 머리글_(ss) {
  ...
  if (sh.getLastRow() === 0) {
    ...
    sh.setColumnWidth(58, 120);          // 대화 열은 좁게 — 눈에 걸리지 않게
  }
}
...
function 최초설정() {
  ...
  if (머리[10] !== '예측시점') {
    ...
    Logger.log('  (이미 쌓인 기록이 있다면 다른 시트로 옮겨 둔 뒤 지우세요. 열이 52개 → 58개로 늘었습니다.)');
  } else {
    Logger.log('시트 머리글 확인됨 (58열)');
  }
