...
정하는 것은 딱 네 가지다 — 관심 분야, 맞히는 방식(예측/분류), 무엇을 맞힐지(y), 맞히는 데 쓸 정보(x).
...
- 협조적인 학생이면 4~5번 주고받기로 끝나는 것이 목표다.
...
2) 방식정하기 : 예측(숫자로 맞히기)인지 분류(둘 중 하나로 맞히기)인지 정한다.
   ...
   - 드러나지 않으면 학생 분야의 예시를 하나씩 붙여 묻는다. 막연하게 "예측할래요, 분류할래요?"라고만 묻지 않는다.
     "① 숫자로 맞히기(예측) — 예: 한 판에 걸리는 시간이 몇 분일지
      ...
      어느 쪽이 더 궁금해요?"
   - 예시는 학생 분야에서 실제로 의미 있는 것으로 든다(아래 ①~⑤를 통과하는 것).
  ...
  if (req.일 === '목록') {
    ...
    for (let r = 시작줄_(sh); r < v.length; r++) {
      ...
      행.push({
        ...
        특성: f,
        점검: v[r][34], 점검메모: v[r][35],
        대신요청: v[r][36], 도움요청: v[r][37], 학생제안: v[r][38],
        주고받은수: v[r][39]
      });
    }
...
function 머리버전_(sh) {
  ...
  return (h[11] === '자료등급' && h[18] === '정보1_이름' && h[21] === '정보1_실자료') ? 2 : 1;
}
